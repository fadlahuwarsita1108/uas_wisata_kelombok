...
public class SharedPreferenceUtility {
     ...
     */
    public static List<WisataKelombok> getAllWisata(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(TRANS_KEY, null);
        List<WisataKelombok> trs = new ArrayList<WisataKelombok>();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    trs.add(WisataKelombok.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllTransaksi(Context ctx, List<WisataKelombok> trs) {
        List<JSONObject> jsonTrs = new ArrayList<JSONObject>();
        JSONArray jsonArr = new JSONArray();
        for (WisataKelombok tr : trs) {
     ...
     */
    public static void addWisata(Context ctx, WisataKelombok tr) {
        List<WisataKelombok> trs = getAllWisata(ctx);
        trs.add(tr);
        saveAllTransaksi(ctx, trs);
    }

    public static void editTransaksi(Context ctx, WisataKelombok tr) {
        List<WisataKelombok> trs = getAllWisata(ctx);
        for (WisataKelombok tre : trs) {
